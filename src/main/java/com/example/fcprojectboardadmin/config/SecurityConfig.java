package com.example.fcprojectboardadmin.config;

import com.example.fcprojectboardadmin.dto.constant.RoleType;
import com.example.fcprojectboardadmin.dto.security.BoardAdminPrincipal;
import com.example.fcprojectboardadmin.dto.security.KakaoOAuth2Response;
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        String[] roleAboveManager = {RoleType.MANAGER.name(), RoleType.DEVELOPER.name(), RoleType.ADMIN.name()};


        return http
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(PathRequest.toStaticResources().atCommonLocations()).permitAll()
                        .mvcMatchers(HttpMethod.POST, "/**").hasAnyRole(roleAboveManager)
                        .mvcMatchers(HttpMethod.DELETE, "/**").hasAnyRole(roleAboveManager)
    ...
    public UserDetailsService userDetailsService(AdminAccountService adminAccountService){
        return username -> adminAccountService
                ...
                .orElseThrow(() -> new UsernameNotFoundException("유저를 찾을 수가 없습니다 - username: " + username));
    }
